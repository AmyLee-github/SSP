...
from utils.tdataloader_copy import get_loader, get_val_loader
from options_copy import TrainOptions
from networks.pf_cam_copy import PF_CAM
...
def train(train_loader, model, optimizer, epoch, save_path, writer):
    ...
    try:
        for i, ((images, images_f, images_b), labels) in enumerate(train_loader, start=1):
            ...
            preds = model(images, images_f, images_b).ravel()
            labels = labels.cuda()
            loss1 = bceLoss()
    ...
    with torch.no_grad():
        for loader in val_loader:
            ...
            for (images, images_f, images_b), labels in val_ai_loader:
                ...
                images_b = images_b.cuda()
                labels = labels.cuda()
                res = model(images, images_f, images_b)
...
if __name__ == '__main__':
    ...
    log_dir = '/hexp/ly/PF_CAM/log/tensorboard/pfb_mask'
