...
def patch_img(img, img_f, img_b, patch_size, height):
    ...
    patch_list = []
    # Randomly select num_patch1 pixel centers within the image dimensions
    centers = [(rd.randint(patch_size // 2, img_width - patch_size // 2),
                rd.randint(patch_size // 2, img_height - patch_size // 2))
               for _ in range(num_patch)]
    # Bind pixel center positions with patches
    for i, center in enumerate(centers):
        x, y = center
        patch = img.crop((x - patch_size // 2, y - patch_size // 2, x + patch_size // 2, y + patch_size // 2))
        patch_list.append([patch, center])
    ...
    img_m = transforms.ToTensor()(img) * mask.unsqueeze(0)
    img_f_m = transforms.ToTensor()(img_f) * mask.unsqueeze(0)
    img_b_m = transforms.ToTensor()(img_b) * mask.unsqueeze(0)
...
def processing(img, img_f, img_b, opt):
    ...
    if opt.isMask:
        img_aug_m, img_f_m, img_b_m = patch_img(img_aug, img_f, img_b, opt.patch_size, opt.trainsize)
    else:
        img_aug_m = transforms.Resize((256, 256))(img_aug)
        img_f_m = transforms.Resize((256, 256))(img_f)
        img_b_m = transforms.Resize((256, 256))(img_b)

    trans_norm_3 = transforms.Normalize([0.485, 0.456, 0.406],
                                        [0.229, 0.224, 0.225])
    trans_norm_1 = transforms.Normalize([0.5], [0.5])

    img_aug_m_n = trans_norm_3(img_aug_m)
    img_f_m_n = trans_norm_1(img_f_m)
    img_b_m_n = trans_norm_1(img_b_m)
    ...
    return img_aug_m_n, img_f_m_n, img_b_m_n
